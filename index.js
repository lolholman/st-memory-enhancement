...
const defaultSettings = {
    muyoo_dataTable: {
        ...
        message_template: 'Please update the following tables based on our conversation:\n$0',
        to_chat_container: '<div class="wide100p padding5 dataBankAttachments">$0</div>',
        deep: 5,
        ...
        refresh_system_message_template: 'You are a table management assistant. Your task is to analyze the conversation and update the tables accordingly. Please follow these rules:\n\n1. Only make necessary changes based on the conversation content\n2. Maintain data consistency and accuracy\n3. Use appropriate table operations (insert, update, delete)\n4. Return the operations in JSON format\n\nCurrent table content:\n$0\n\nRecent conversation:\n$1',
        refresh_user_message_template: 'Please analyze the conversation and update the tables accordingly. Return the operations in JSON format.',
        tableStructure: [
            {
                tableIndex: 0,
                name: "Time-Space Table",
                columns: ["Time", "Location", "Description"],
                note: "Record important time and space information",
                enable: true,
                toChat: true,
                tableRender: null
            },
            {
                ...
                note: "Record character personality traits",
                enable: true,
                toChat: true,
                tableRender: null
            },
            {
                tableIndex: 2,
                name: "Character Relationships Table",
                columns: ["Character", "Relationship", "Description"],
                note: "Record character relationships",
                enable: true,
                toChat: true,
                tableRender: null
            },
            {
                tableIndex: 3,
                name: "Tasks and Agreements Table",
                columns: ["Task", "Status", "Description"],
                note: "Record tasks, commands, or agreements",
                enable: true,
                toChat: true,
                tableRender: null
            },
            {
                ...
                note: "Record important historical events",
                enable: true,
                toChat: true,
                tableRender: null
            },
            {
                ...
                columns: ["Item", "Owner", "Description"],
                note: "Record important items",
                enable: true,
                toChat: true,
                tableRender: null
            }
...
async function resetSettings() {
    ...
    const confirmation = await callGenericPopup(tableInitPopup, POPUP_TYPE.CONFIRM, '', { okButton: "Continue", cancelButton: "Cancel" });
    if (confirmation) {
        // Never simplify the following ternary expressions and assignment order!!! Otherwise, the reset settings will not work correctly
        // Check if all basic settings should be reset (this check must be on the first line)
        ...
        // The following assignment order can be changed
        // Check if message template should be reset
        ...
        // Check if table organization prompt should be reset
        ...
        // Check if all table structures should be reset
...
function getRefreshTableConfigStatus() {
    ...
    return `<div class="wide100p padding5 dataBankAttachments">
                <span>Will reorganize the tables, continue?</span><br><span style="color: rgb(211 39 39)">(Recommended to backup data before reset)</span>
                <br><div id="config_sheet_container" style="justify-content: center; display: flex; margin: 10px;">
                    <table class="table table-bordered table-striped">
                        <thead><tr><th>Configuration Item</th><th style="padding: 0 20px">Configuration Value</th></tr></thead>
                        <tbody>
                        <tr> <td>Chat History to Consider</td> <td>${clearUpStairs} messages</td> </tr>
                        <tr> <td>Prevent AI Deletion</td> <td>${isIgnoreDel ? 'Yes' : 'No'}</td> </tr>
                        <tr> <td>API Used</td> <td>${isUseMainAPI ? 'Main API' : 'Custom API'}</td> </tr>
                        ${isUseMainAPI ? '' : `
...
async function refreshTableActions() {
    ...
    const confirmation = await callGenericPopup(tableRefreshPopup, POPUP_TYPE.CONFIRM, '', { okButton: "Continue", cancelButton: "Cancel" });
    ...
    const loadingToast = toastr.info(isUseMainAPI
        ? 'Organizing tables using [Main API]...'
        : 'Organizing tables using [Custom API]...',
        '',
        { timeOut: 0 }
    );
    try {
        ...
        let originText = '<Table Content>\n' + latestTables
            .map(table => table.getTableText(['title', 'node', 'headers', 'rows']))
            .join("\n");

        // Get the most recent clear_up_stairs chat messages
        let chat = getContext().chat;
        let lastChats = '';
        if (chat.length < extension_settings.muyoo_dataTable.clear_up_stairs) {
            toastr.success(`Current chat history has only ${chat.length} messages, less than the set ${extension_settings.muyoo_dataTable.clear_up_stairs} messages`);
            for (let i = 0; i < chat.length; i++) {  // Start from 0 to traverse all existing messages
        ...
        // Search for $0 and $1 fields in userPrompt, replace $0 with originText, $1 with lastChats
        ...
        if (isUseMainAPI) {
            ...
            console.log('Original response content:', response);

            // Clean response content
            cleanContent = response
                .replace(/```json|```/g, '')
                .replace(/^[^[]*(\[.*\])[^]]*$/s, '$1') // Avoid various presets adding extra content outside, only extract JSON part
                .trim();
        } else {
            // Custom API
            const USER_API_URL = extension_settings.IMPORTANT_USER_PRIVACY_DATA.custom_api_url;
            const USER_API_KEY = await getDecryptedApiKey(); // Use decrypted key
            const USER_API_MODEL = extension_settings.IMPORTANT_USER_PRIVACY_DATA.custom_model_name;

            if (!USER_API_URL || !USER_API_MODEL) { //Removed !USER_API_KEY check as local models and some channels don't need API KEY
                toastr.error('Please fill in complete custom API configuration');
                return;
            }
